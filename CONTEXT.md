...
**Trusted Runtime**:
The external, agent-unwritable launcher and extension layer that enforces sandboxing for every agent run.
_Avoid_: putting sandbox authority inside an agent-owned directory
...
- The **Trusted Runtime** launches agents but is not owned by any agent.
- Every standalone agent run must pass through the **Trusted Runtime** before tools or shell commands execute.
...
- The trusted launcher is named `papa` and exports explicit path variables with `_DIR` suffixes, such as `PI_AGENT_MEMORY_DIR`, `PI_AGENT_WORLD_DIR`, `PI_AGENT_SESSIONS_DIR`, and `PI_AGENT_DECISIONS_DIR`, for skills and extensions.
